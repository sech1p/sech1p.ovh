<header id="logo">sech1p's corner</header>
<nav id="links">
	<ul>
		<li><a href="index.php">Home</a></li>
		<li><a href="https://elizaemiliaa.wordpress.com">Blog (WordPress)</a></li>
		<li><a href="https://sech1p.dreamwidth.org">Blog (Dreamwidth)</a></li>
		<li><a href="about.php">About me</a></li>
		<li><a href="contact.php">Contact & Social Medias</a></li>
	</ul>
</nav>
<div id="menu">
	<div class="menu">
		<div class="menu-header" id="menu-header">Menu</div>
			<nav class="menu-content">
			    <ul>
					<li><b>Links</b></li>
					<li><a href="https://sech1p.art">Portfolio</a></li>
					<li><a href="https://sech1p.link">My card</a></li>
					<li><a href="https://t.me/sech1p">Telegram</a></li>
					<li><b>Friends</b></li>
				    <li><a href="https://ponymare.ovh"><img src="https://ponymare.ovh/res/trixie.gif" title="my cute gf emilia :3"></a></li>
					<li><a href="https://anemonik.neocities.org"><img src="resources/anemonek.png" title="Anemonik"></a></li>
					<li><a href="https://karibooru.love"><img src="resources/kariga.png" title="Karibooru"></a></li>
					<li><b>My Button</b></li>
					<li><img src="resources/sech1p.gif" /></li>
        			<li><b>Buttons (88x31)</b></li>
					<!--
						Credits:
						Skye (https://skyeweeb.weebly.com)
    					Vidapon (https://vidapon.net)
						capstasher (https://capstasher.neocities.org)
					-->
	        		<li><img src="resources/im_high.gif" /></li>
		        	<li><img src="resources/transgender.gif" /></li>
					<li><img src="resources/flag_lesbian.png" /></li>
		        	<li><img src="resources/any_browser.gif" /></li>
					<li><img src="resources/quake.gif" /></li>
    			    <li><img src="resources/anime_gay.gif" /></li>
		    	</ul>
		    </nav>
		</div>
	</div>